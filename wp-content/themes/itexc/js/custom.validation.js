...
$(function() {
	...
	if($(this).attr('id') == 'wpsearchform') return;
    ...
    $.validator.addMethod("validateInternationalphone", function(value, element) {
        var regEx = /^([+])+([0-9])+$/;
        return regEx.test(value);
    }, "Please enter the valid international phone no");
    
    // Desk phone Validation - optional field
    $.validator.addMethod("validateDeskPhone", function(value, element) {
		if(value){
			var regEx = /^[0-9-+ ]+$/;
			return regEx.test(value);
			}
    ...
    $.validator.addMethod("isPartnerAgreementAccepted", function(value, element) {
